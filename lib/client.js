...
if (!api.http.STATUS_CODES[508]) {
  api.http.STATUS_CODES[508] = 'Loop Detected';
}
...
class Client {
  ...
  constructor(application, req, res) {
    ...
    if (req.parameters) {
      this.parameters = { ...req.parameters, ...this.query };
    } else {
  ...
  //   output <string> | <Buffer> | <Object> { stats, compressed, data }
    ...
    // TODO: Temporary disable sessions for refactoring
    // this.saveSession(() => {
    {
      this.sendCookie();
  ...
  template(data, file, cursor, callback) {
    ...
    if (this.logged) {
      if (this.session && this.session.group) {
        files.push(file + '.' + this.session.group);
      }
  ...
  include(data, filePath, cursor, callback) {
    ...
    const relPath = application.relative(filePath);
    const cache = application.cache.templates.get(relPath);
    if (cache) {
    ...
    api.fs.readFile(filePath, 'utf8', (err, tpl) => {
      ...
      application.cache.templates.set(relPath, tpl);
